...
undoStack[6] = 删除选中元素（含状态变化）
```

**说明**：历史条目数与用户视觉操作数不完全对齐，这是因为：在两次元素变化 commit 之间若存在单独的选择状态变化 commit，则会产生独立历史条目；删除操作本身也会改变当前选择状态，这些都被精确记录以保证撤销时的状态完整性。

### 2.2 选择变化的记录机制

**选择状态变化的记录方式取决于 commit 时机**：
...
**记录规则**：
- 若在两次元素变化 commit 之间存在单独的选择变化 commit，则选择变化独立记录
- 若选择变化与元素变化在同一个 commit 周期内发生，则合并记录到同一个 delta 条目中
...
1. **精确的状态恢复**：被记录的每个状态变化粒度都足够细，撤销时能精确恢复
...
1. **分组原则**：基于用户交互的原子性，选择状态变化的记录方式取决于 commit 时机——两次元素变化之间的单独选择 commit 独立记录，否则与元素变化合并记录
